...
class SumOfNUsingForkJoin {
	...
	static class RecursiveSumOfN extends RecursiveTask<Long> {
		...
		// (remember that we want to divide the summation task equally among
		// NUM_THREADS)
		// then, sum the range of numbers from..to using a simple for loop
		// otherwise, fork the range and join the results
		public Long compute() {
			if ((to - from) <= N / NUM_THREADS) {
				// the range is something that can be handled by a thread, so do
				// summation
				long localSum = 0;
				...
				//System.out.printf(
				//		"\t Summing of value range %d to %d is %d %n", from,
				//		to, localSum);
				return localSum;
			} else { // no, the range is big for a thread to handle, so fork the
						// computation
			// we find the mid-point value in the range from..to
				...
				// determine the computation for first half with the range
				// from..mid
				RecursiveSumOfN firstHalf = new RecursiveSumOfN(from, mid);
				// now, fork off that task
				firstHalf.fork();
				// determine the computation for second half with the range
				// mid+1..to
	...
	public static void main(String[] args) {
		...
		System.out.printf(
				"Sum for range 1..%d; computed sum = %d, time taken in thread computation = %d %n",
				N, computedSum, (stop - start));
	}
}